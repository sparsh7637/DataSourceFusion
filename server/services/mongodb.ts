...
export class MongoDBService {
  ...
  private setupSampleData() {
    ...
    const inventoryData = [
      {
        _id: "7b1b2c3d4e5f6a1b2c3d4e5f",
        productId: "PROD-001",
        quantity: 120,
        location: "Warehouse A"
      },
      {
        _id: "7b1b2c3d4e5f6a1b2c3d4e60",
        productId: "PROD-002",
        quantity: 75,
        location: "Warehouse B"
      },
      {
        _id: "7b1b2c3d4e5f6a1b2c3d4e61",
        productId: "PROD-003",
        quantity: 30,
        location: "Warehouse A"
      }
    ...
    this.data.set("inventory", inventoryData);
  }
  ...
  async executeQuery(collectionName: string, queryParams: any): Promise<any[]> {
    ...
    console.log(`Executing MongoDB query on collection: ${collectionName}`, queryParams);
      ...
      try {
        ...
        if (queryParams && typeof queryParams === 'object') {
          ...
          if (queryParams.filters && Array.isArray(queryParams.filters)) {
            filter = this.buildMongoQuery(queryParams.filters);
          }
          ...
          if (queryParams.orderBy && typeof queryParams.orderBy === 'object') {
            ...
            for (const field in queryParams.orderBy) {
              options.sort[field] = queryParams.orderBy[field] === 'desc' ? -1 : 1;
            }
          }
    ...
    if (this.data.has(collectionName)) {
      return this.filterStoredData(this.data.get(collectionName) || [], queryParams);
    }
